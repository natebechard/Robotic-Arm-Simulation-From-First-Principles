...
Init_values = [-np.pi/2,0,-np.pi/2,0,-np.pi/2,0,-np.pi/2,0] # position1, velocity1, position2, velocity2, ....., positionn, velocityn
...
def slopes(t, Input):     # Y is a vector of shape [x1, x2,..., xn]
    output = np.zeros(segment_num*2)

    #computes the distance between joints in an arm
    for joint_computed in range(segment_num):  # iterate through the segments to compute the rotational inertia on each joint
        rotational_inertia = 0 #rotational inertia WRT the joint being computed
        net_torque_on_joint = 0 #net torque on the joint
        for mass_computed in range((segment_num-1),(joint_computed-1),-1):
            lx, ly = 0, 0
            dist = 0
           ...
           # print("distance from motor " + str(joint_computed) + " to mass " + str(mass_computed) + " = " + str(dist))+ "the mass here is: " + str(Masses[mass_computed])
            rotational_inertia += Masses[mass_computed] * dist**2 # this is the rotational inertia computation. Mass*radius^2
            net_torque_on_joint += Masses[mass_computed]*grav*dist*(lx/dist) # lx/dist is the ratio between the adjacent and opposite of the angle at which torque is applied. It is therefore the cosine
       ...
       # print(((Torques[joint_computed] - net_torque_on_joint) / rotational_inertia)  - Bs[joint_computed]*Input[2*joint_computed + 1])
        ...
        output[joint_computed*2 +1] = ((compute_torque(t)[joint_computed] - net_torque_on_joint) / rotational_inertia)  - Bs[joint_computed]*Input[2*joint_computed + 1] #x2'
...
x_coords = np.zeros((segment_num+1, len(sol.y[0])))
y_coords = np.zeros((segment_num+1, len(sol.y[0])))

x = 0
y = 0

for joint_num in range(segment_num):
